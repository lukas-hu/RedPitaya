...
def pid_set_values(res, sp, kp, ki, kd, d1, d2, hv):
	...
	params.Ki12 = ki
	params.Kd12 = kd
	params.Kd21 = int(d1/8)		# d1 is input in nanoseconds, one clock cycle is 8ns. Delay in register should be given in clock cycles
	params.Ki21 = int(d2/8)		# d2 is input in nanoseconds, one clock cycle is 8ns. Delay in register should be given in clock cycles
	return
...
def pid_get_values():
	...
	return [bool(res), float(sp), int(params.Kp12), int(params.Ki12), int(params.Kd12), float(params.Kd21), float(params.Ki21)]
