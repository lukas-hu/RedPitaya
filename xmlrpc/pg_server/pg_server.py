...
waveform1_array1 = np.recarray(1, waveform_regset1, buf=m21)
waveform11 = waveform1_array1[0]

m22 = mmap.mmap(fileno=fd, length=mmap.PAGESIZE, offset=0x40611000)
waveform1_array2 = np.recarray(1, waveform_regset2, buf=m22)
waveform12 = waveform1_array2[0]

m23 = mmap.mmap(fileno=fd, length=mmap.PAGESIZE, offset=0x40612000)
waveform1_array3 = np.recarray(1, waveform_regset3, buf=m23)
waveform13 = waveform1_array3[0]

m24 = mmap.mmap(fileno=fd, length=mmap.PAGESIZE, offset=0x40613000)
waveform1_array4 = np.recarray(1, waveform_regset4, buf=m24)
waveform14 = waveform1_array4[0]

m31 = mmap.mmap(fileno=fd, length=mmap.PAGESIZE, offset=0x40620000)
waveform2_array1 = np.recarray(1, waveform_regset1, buf=m31)
waveform21 = waveform2_array1[0]

m32 = mmap.mmap(fileno=fd, length=mmap.PAGESIZE, offset=0x40621000)
waveform2_array2 = np.recarray(1, waveform_regset2, buf=m32)
waveform22 = waveform2_array2[0]

m33 = mmap.mmap(fileno=fd, length=mmap.PAGESIZE, offset=0x40622000)
waveform2_array3 = np.recarray(1, waveform_regset3, buf=m33)
waveform23 = waveform2_array3[0]

m34 = mmap.mmap(fileno=fd, length=mmap.PAGESIZE, offset=0x40623000)
waveform2_array4 = np.recarray(1, waveform_regset4, buf=m34)
waveform24 = waveform2_array4[0]
...
def pg_set_values(res, amp, wave, ki, d1, d2, hv, ch1mode, offsetmode, offset):
	...
	for i in range(0,1024):
		setattr(waveform11,str(i),int(round((amp*calib[1] + calib[0])*wave[i])))
		setattr(waveform21,str(i),int(16383*wave[i]))
		
	for i in range(1024,2048):
		setattr(waveform12,str(i),int(round((amp*calib[1] + calib[0])*wave[i])))
		setattr(waveform22,str(i),int(16383*wave[i]))
		
	for i in range(2048,3072):
		setattr(waveform13,str(i),int(round((amp*calib[1] + calib[0])*wave[i])))
		setattr(waveform23,str(i),int(16383*wave[i]))
		
	for i in range(3072,4096):
		setattr(waveform14,str(i),int(round((amp*calib[1] + calib[0])*wave[i])))
		setattr(waveform24,str(i),int(16383*wave[i]))
	 #CAN MAKE MORE EFFICIENT BY USING ONLY ONE REGSET!!!
